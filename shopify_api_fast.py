#!/usr/bin/env python3
"""
shopify_api_fast.py — High-Performance Async Shopify Checkout API Server
=========================================================================
Replaces the synchronous Flask API with a fully-async aiohttp server that
can handle 50+ concurrent checkouts using the proven process_card() logic.

Endpoints:
  GET /shopify?site=<url>&cc=<CC|MM|YY|CVV>&proxy=<host:port:user:pass>&variant=<id>
  GET /health

Configuration: see HOST / PORT / MAX_CONCURRENT_CHECKS constants below.
"""
...
# Flask removed — replaced by aiohttp
import os
import time
...
async def fetch_products(domain, proxy_str=None, _connector=None):
    try:
        if not domain.startswith('http'):
            domain = "https://" + domain
        
        proxy = parse_proxy(proxy_str) if proxy_str else None
        _fp_own = _connector is None
        _fp_conn = aiohttp.TCPConnector(ssl=False) if _fp_own else _connector
        _fp_session = aiohttp.ClientSession(
            connector=_fp_conn, connector_owner=_fp_own,
            timeout=aiohttp.ClientTimeout(total=10)
        )
        try:
            async with _fp_session.get(f"{domain}/products.json", proxy=proxy, timeout=10) as resp:
                if resp.status != 200:
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None, _connector=None):
    gateway = "UNKNOWN"
    total_price = "0.00"
    currency = "USD"
    ...
    try:
        ...
        _pc_own = _connector is None
        _pc_conn = aiohttp.TCPConnector(ssl=False) if _pc_own else _connector
        _pc_session = aiohttp.ClientSession(
            connector=_pc_conn, connector_owner=_pc_own,
            timeout=aiohttp.ClientTimeout(total=30)
        )
        try:
            session = _pc_session  # use managed session
            url = ourl
            ...
            await _pc_session.close()
    except Exception as e:
        return False, f"Error Processing Card: {str(e)}", gateway, total_price, currency

# ─────────────────────────────────────────────────────────────────────────────
# shopify_api_fast.py — aiohttp HIGH-PERFORMANCE wrapper
# ─────────────────────────────────────────────────────────────────────────────
...
logger = logging.getLogger("shopify_api_fast")

# ── Global state (initialised in on_startup) ─────────────────────────────────
_global_sem: asyncio.Semaphore = None          # filled in startup
_domain_sems: dict = {}                         # domain -> Semaphore(2)
_domain_sems_lock: asyncio.Lock = None
_shared_connector: aiohttp.TCPConnector = None  # shared TCP connection pool
_active_count: int = 0


async def _get_domain_sem(domain: str) -> asyncio.Semaphore:
    """Return (and lazily create) the per-domain Semaphore."""
    async with _domain_sems_lock:
        if domain not in _domain_sems:
            _domain_sems[domain] = asyncio.Semaphore(MAX_DOMAIN_CONCURRENT)
...
async def handle_shopify(request: web.Request) -> web.Response:
    ...
    site       = qs.get("site", "").strip()
    cc_string  = qs.get("cc", "").strip()
    proxy_str  = qs.get("proxy", "").strip() or None
    variant_id = qs.get("variant", "").strip() or None
    ...
    if len(parts) != 4:
        return web.json_response(
            {"Status": False, "Response": "Invalid CC format — use CC|MM|YY|CVV"}, status=400
        )
    ...
    try:
        ...
        return web.json_response({
            "Status": False, "Response": "REQUEST_TIMEOUT",
            "Gateway": "UNKNOWN", "Price": 0.0, "cc": cc_string
        })
    except Exception as e:
        elapsed = time.time() - start
        logger.error(f"[{domain}] EXCEPTION after {elapsed:.1f}s: {e}")
        return web.json_response({
            "Status": False, "Response": f"ERROR: {str(e)[:120]}",
            "Gateway": "UNKNOWN", "Price": 0.0, "cc": cc_string
        }, status=500)
    ...
    logger.info(
        f"[{domain}] {elapsed:.2f}s | gate={gateway} | {clean} | {'HIT' if success else 'miss'}"
    )
...
async def on_startup(app: web.Application):
    global _global_sem, _domain_sems_lock, _shared_connector
    _global_sem = asyncio.Semaphore(MAX_CONCURRENT_CHECKS)
    _domain_sems_lock = asyncio.Lock()
    # Shared TCP connector: 300 total connections, DNS cache 300s
    _shared_connector = aiohttp.TCPConnector(
        limit=300, ssl=False, ttl_dns_cache=300, enable_cleanup_closed=True
    )
    logger.info(f"shopify_api_fast started on {HOST}:{PORT} | max={MAX_CONCURRENT_CHECKS}")


async def on_cleanup(app: web.Application):
    if _shared_connector:
        await _shared_connector.close()
    logger.info("shopify_api_fast stopped")
...
def main():
    ...
    web.run_app(app, host=HOST, port=PORT, access_log=None, print=logger.info)
